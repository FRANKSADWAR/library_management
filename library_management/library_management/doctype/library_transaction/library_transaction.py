...
class LibraryTransaction(Document):
	def before_submit(self):
		if self.type == "Issue":
			...
			## Set the article status to Available whenever it has been returned
			article = frappe.get_doc("Article",self.article)
	...
	def validate_issue(self):
		...
		article = frappe.get_doc("Article",self.article)
		##	article cannot be issued if its already issued
		if article.status == "Issued":
			frappe.throw("Article is already issued to another member")

	def validate_return(self):
		article = frappe.get_doc("Article",self.article)
		## Article cannot be returned if it has not been issued first
